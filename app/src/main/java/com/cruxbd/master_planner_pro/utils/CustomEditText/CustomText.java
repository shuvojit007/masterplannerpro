...
public class CustomText extends EditText implements TextWatcher {
    ...
    private void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.CustomText);
        bulletColor = array.getColor(R.styleable.CustomText_bulletColor, 0);
        bulletRadius = array.getDimensionPixelSize(R.styleable.CustomText_bulletRadius, 0);
        bulletGapWidth = array.getDimensionPixelSize(R.styleable.CustomText_bulletGapWidth, 0);
        historyEnable = array.getBoolean(R.styleable.CustomText_historyEnable, true);
        historySize = array.getInt(R.styleable.CustomText_historySize, 100);
        linkColor = array.getColor(R.styleable.CustomText_linkColor, 0);
        linkUnderline = array.getBoolean(R.styleable.CustomText_linkUnderline, true);
        quoteColor = array.getColor(R.styleable.CustomText_quoteColor, 0);
        quoteStripeWidth = array.getDimensionPixelSize(R.styleable.CustomText_quoteStripeWidth, 0);
        quoteGapWidth = array.getDimensionPixelSize(R.styleable.CustomText_quoteCapWidth, 0);
        array.recycle();
    ...
    protected void styleInvalid(int style, int start, int end) {
        ...
        List<CustomPart> list = new ArrayList<>();

        for (StyleSpan span : spans) {
            if (span.getStyle() == style) {
                list.add(new CustomPart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
                getEditableText().removeSpan(span);
            }
        }

        for (CustomPart part : list) {
            if (part.isValid()) {
    ...
    protected void bulletValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (bulletStart < bulletEnd) {
                getEditableText().setSpan(new CustomBulletSpan(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void quoteValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (quoteStart < quoteEnd) {
                getEditableText().setSpan(new CustomQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void linkValid(String link, int start, int end) {
        ...
        linkInvalid(start, end);
        getEditableText().setSpan(new CustomURLSpan(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public void fromHtml(String source) {
        ...
        builder.append(CustomParser.fromHtml(source));
        switchToKnifeStyle(builder, 0, builder.length());
        setText(builder);
    }

    public String toHtml() {
        return CustomParser.toHtml(getEditableText());
    }
    ...
    protected void switchToKnifeStyle(Editable editable, int start, int end) {
        ...
        for (BulletSpan span : bulletSpans) {
            ...
            editable.setSpan(new CustomBulletSpan(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (QuoteSpan span : quoteSpans) {
            ...
            spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
            editable.removeSpan(span);
            editable.setSpan(new CustomQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (URLSpan span : urlSpans) {
            ...
            int spanEnd = editable.getSpanEnd(span);
            editable.removeSpan(span);
            editable.setSpan(new CustomURLSpan(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
}
